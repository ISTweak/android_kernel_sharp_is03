...
struct clkctl_acpu_speed acpu_freq_tbl_998[] = {
	...
	{ 1, 1036800, ACPU_PLL_3, 0, 0, 0, 0, 128000, 1, 0x1B, 1320},
	{ 1, 1075200, ACPU_PLL_3, 0, 0, 0, 0, 128000, 1, 0x1C, 1340},
	{ 1, 1113600, ACPU_PLL_3, 0, 0, 0, 0, 259200, 1, 0x1D, 1380},
	{ 1, 1152000, ACPU_PLL_3, 0, 0, 0, 0, 259200, 1, 0x1E, 1450},
	{ 0, 1190400, ACPU_PLL_3, 0, 0, 0, 0, 259200, 1, 0x1F, 1500},
	{ 0, 1228800, ACPU_PLL_3, 0, 0, 0, 0, 259200, 1, 0x20, 1500},
	{ 0, 1267200, ACPU_PLL_3, 0, 0, 0, 0, 259200, 1, 0x21, 1500},
...
static struct cpufreq_frequency_table freq_table[24];
...
ssize_t acpuclk_get_vdd_levels_str(char *buf)
{
	...
	if (buf) {
		mutex_lock(&drv_state.lock);
		for (i = 0; acpu_freq_tbl_998[i].acpuclk_khz; i++) {
			if (acpu_freq_tbl_998[i].use_for_scaling)
				len += sprintf(buf + len, "%8u: %4d\n", acpu_freq_tbl_998[i].acpuclk_khz, acpu_freq_tbl_998[i].vdd);
		}
		mutex_unlock(&drv_state.lock);
	}
	return len;
...
{
	int i;
	vdd = vdd / 10 * 10; //! regulator only accepts multiples of 10 (mV)
	mutex_lock(&drv_state.lock);
	for (i = 0; acpu_freq_tbl_998[i].acpuclk_khz; i++) {
		if (acpu_freq_tbl_998[i].use_for_scaling) {
			if (acpu_khz == 0)
				acpu_freq_tbl_998[i].vdd = min(max((acpu_freq_tbl_998[i].vdd + vdd), VOLTAGE_MIN), VOLTAGE_MAX);
			else if (acpu_freq_tbl_998[i].acpuclk_khz == acpu_khz)
				acpu_freq_tbl_998[i].vdd = min(max(vdd, VOLTAGE_MIN), VOLTAGE_MAX);
		}
	}
	mutex_unlock(&drv_state.lock);
}
