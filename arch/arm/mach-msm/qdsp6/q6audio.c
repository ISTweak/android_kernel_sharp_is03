...
static struct q6_hw_info q6_audio_hw[Q6_HW_COUNT] = {
	...
	[Q6_HW_I2S_RX_HEADSET] = {
		.min_gain = -1000,
		.max_gain = 500,
	},
	[Q6_HW_I2S_RX_NO_MIC_HEADSET] = {
		.min_gain = -1000,
		.max_gain = 500,
	},
	[Q6_HW_LINE_IN_MIC] = {
	...
	[Q6_HW_I2S_RX_FAREND_TERMINAL] = {
		.min_gain = -1000,
		.max_gain = 500,
	},
	[Q6_HW_LINE_IN_HANDSET] = {
		.min_gain = -400,
		.max_gain = 1100,
	},
...
static struct q6_hw_info q6_audio_device_vol[Q6_HW_COUNT] = {
	[Q6_HW_HANDSET] = {
		.min_gain = -700,
		.max_gain = 200,
	},
...
int set_device_volume( uint32_t device_id, uint32_t cad_id, int level );
int q6_device_volume_ex(uint32_t device_id, uint32_t cad_id, int level);
int set_device_mute( uint32_t mute );

static uint32_t dev_vol_device_id = ADSP_AUDIO_DEVICE_ID_HANDSET_SPKR;
static uint32_t dev_vol_cad_id    = 0;
static int      dev_vol_level     = 100;
static uint32_t dev_mute     = 0;
...
static void _audio_tx_clk_enable(void)
{
	...
	case Q6_ICODEC_TX:
		...
		if (icodec_tx_clk_refcount == 1) {
			...
			// The icodec tx clock should be always equal to AFE configured value.
			if (tx_clk_freq > 16000)
				      ...
				      uint32_t channels, uint32_t flags, uint32_t acdb_id)
{
	...
	if (ac->flags & AUDIO_FLAG_WRITE){
	    set_device_volume(dev_vol_device_id, dev_vol_cad_id, dev_vol_level );
		set_device_mute( dev_mute );
	}
...
int q6audio_set_device_volume( uint32_t device_id, uint32_t cad_id, int level )
{
	int ret = 0;

    TRACE("q6audio_set_device_volume() start audio_rx_path_refcount(%d)\n", audio_rx_path_refcount);

	dev_vol_device_id = device_id;
	dev_vol_cad_id    = cad_id;
	dev_vol_level     = level;

	if( audio_rx_path_refcount > 0 ){
		ret = set_device_volume( device_id, cad_id, level );
		if( ret != 0 ){
			pr_err("q6audio_set_device_volume: set_device_volume Error \n");
			return ret;
		}
		ret = set_device_mute( dev_mute );
	}
    
    TRACE("q6audio_set_device_volume() end return=%d\n", ret);
	return ret;
}

int q6_device_volume_ex(uint32_t device_id, uint32_t cad_id, int level)
{
	struct q6_device_info *di = q6_lookup_device(device_id, cad_id);
	struct q6_hw_info *hw;
	hw = &q6_audio_device_vol[di->hw];
	return hw->min_gain + ((hw->max_gain - hw->min_gain) * level) / 100;
}
...
int set_device_volume( uint32_t device_id, uint32_t cad_id, int level )
{
    ...
    TRACE("set_device_volume() start device_id(0x%x) cad_id(0x%x) level(%d)\n",device_id, cad_id, level);
	if(device_id == 0)
		return 0;
	...
	if( dev_mute == 0 ){
		mutex_lock(&audio_path_lock);
		adev = device_id;
		vol = q6_device_volume_ex(device_id, cad_id, level );
		audio_rx_mute(ac_control, adev, 0);
		TRACE("set_device_volume() get setting volume(%d) \n",vol);
		audio_rx_volume(ac_control, adev, vol);
		rx_vol_level = level;
...
int q6audio_set_device_mute( uint32_t mute )
{
	int ret = 0;

    TRACE("q6audio_set_device_mute() start audio_rx_path_refcount(%d) mute(%d)\n", audio_rx_path_refcount, mute);

	if( audio_rx_path_refcount > 0 ){
		ret = set_device_mute( mute );


		if( dev_mute != 0 && mute == 0 ){
			if( dev_vol_device_id > 0 ){
				dev_mute = mute;
				set_device_volume( dev_vol_device_id, dev_vol_cad_id, dev_vol_level );
			}
		}
    ...
    TRACE("q6audio_set_device_mute() end return=%d\n", ret);
	return ret;
}

int set_device_mute( uint32_t mute )
{
    TRACE("set_device_mute: start audio_rx_device_id(0x%x) mute(%d)\n", audio_rx_device_id, mute);
    ...
    TRACE("set_device_mute: call audio_rx_mute()\n");
	audio_rx_mute( ac_control, audio_rx_device_id, mute );
	mutex_unlock(&audio_path_lock);
	return 0;
