/* linux/arch/arm/mach-msm/rpc_pmapp.c  (rpc for pmapp)
 *
...
#ifdef CONFIG_SH_USB_CUST
#define GPIO_USB_PHY_CLK_ENABLE			(98)
#define GPIO_USB_PHY_VREG_ENABLE		(104)
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
	if (enable) {
		gpio_set_value(GPIO_USB_PHY_CLK_ENABLE, enable);
		gpio_set_value(GPIO_USB_PHY_VREG_ENABLE, enable);
	} else {
		gpio_set_value(GPIO_USB_PHY_VREG_ENABLE, enable);
		gpio_set_value(GPIO_USB_PHY_CLK_ENABLE, enable);
	}
