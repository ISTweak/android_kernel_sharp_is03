...
static void sh_battery_delay_init( struct work_struct* work_p );

static int sh_battery_delay_init_sub( void );
...
static struct delayed_work sh_battery_work;
...
static int sh_battery_probe( struct platform_device* dev_p )
{
  ...
  INIT_DELAYED_WORK(&sh_battery_work,sh_battery_delay_init);

  sh_battery_delay_init(NULL);
...
static void sh_battery_delay_init( struct work_struct* work_p )
{
  if(sh_battery_delay_init_sub() != 0)
  {
    schedule_delayed_work(&sh_battery_work,msecs_to_jiffies(100));
  }
}
