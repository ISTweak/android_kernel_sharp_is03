...
#endif	/* CONFIG_KEYBOARD_I2C_MSM */
#ifdef CONFIG_LYNCIS_TOUCHSCREEN
...
#ifdef CONFIG_LYNCIS_SENSOR
#include <linux/i2c/lis331.h>
#endif
...
#ifdef CONFIG_MACH_DECKARD
#define MSM_PMEM_SF_SIZE 0x2600000
#endif

#ifdef CONFIG_MACH_LYNCIS
...
#ifdef CONFIG_LYNCIS_VIBRATOR
extern void msm_init_pmic_vibrator( void );
#endif

#ifdef CONFIG_USB_FUNCTION
#if 0 /* refer to msm_hsusb_map.h */ /* shusb */
...
static struct msm_handset_platform_data sh_pm_platform_data = {
	.hs_name = "SH_pm_key",
};

static struct msm_handset_platform_data sh_hs_platform_data = {
	.hs_name = "SH_headset_key",
};

static struct msm_handset_platform_data sh_matrix_platform_data = {
	.hs_name = "SH_matrix_keypad",
};
...
static struct platform_device sh_pm_device = {
	...
	.id     = -1,
	.dev    = {
		.platform_data = &sh_pm_platform_data,
	},
};
...
static struct platform_device sh_hs_device = {
	...
	.id     = -1,
	.dev    = {
		.platform_data = &sh_hs_platform_data,
	},
};

static struct platform_device sh_matrix_device = {
	.name   = "SH_matrix_keypad",
	.id     = -1,
	.dev    = {
		.platform_data = &sh_matrix_platform_data,
	},
};

#ifdef CONFIG_USB_FS_HOST
static struct msm_gpio fsusb_config[] = {
#if 0 /* PENDING: these GPIOs are used by Bluetooth */ /* shusb */
...
#endif
#else
#ifndef CONFIG_LYNCIS_TOUCHSCREEN
static struct msm_gpio tps_spi_gpio_config_data[] = {
	{ GPIO_CFG( SH_TOUCH_IRQ,      0, GPIO_INPUT,  GPIO_PULL_UP, GPIO_2MA), "touch_irq"   },
	{ GPIO_CFG( SH_TOUCH_HSSP_CLK, 0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), "touch_hssp_clk" },
	{ GPIO_CFG( SH_TOUCH_HSSP_DATA,0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), "touch_hssp_data"},
	{ GPIO_CFG( SH_TOUCH_RESET,    0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), "touch_reset" },
	{ GPIO_CFG( SH_TOUCH_STBY,     0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA), "touch_standby"  },
};

static int msm_tps_gpio_setup(void)
{
	int rc;

	rc = msm_gpios_request_enable(tps_spi_gpio_config_data,
		ARRAY_SIZE(tps_spi_gpio_config_data));

	return rc;
}

static void msm_tps_gpio_teardown(void)
{
	msm_gpios_disable_free(tps_spi_gpio_config_data,
		ARRAY_SIZE(tps_spi_gpio_config_data));
}

static struct msm_sh_spitps_platform_data tps_pdata = {
	.gpio_irq       = SH_TOUCH_IRQ,
	.gpio_hssp_clk  = SH_TOUCH_HSSP_CLK,
	.gpio_hssp_data = SH_TOUCH_HSSP_DATA,
	.gpio_reset     = SH_TOUCH_RESET,
	.gpio_standby   = SH_TOUCH_STBY,
	.gpio_setup     = msm_tps_gpio_setup,
	.gpio_teardown  = msm_tps_gpio_teardown,
};
...
static struct spi_board_info msm_spi_board_info[] __initdata = {
	{
		.modalias	= SH_TOUCH_SPI_DEVNAME,
		.mode		= SPI_MODE_1,
		.irq		= MSM_GPIO_TO_INT(92),
		.bus_num	= 0,
		.chip_select	= 0,
		.max_speed_hz	= 1000000, /* 1.0Mbps */
		.platform_data	= &tps_pdata,
	}
};
...
#else
static struct msm_gpio shtps_spi_gpio_config_data[] = {
	{ GPIO_CFG( 92, 0, GPIO_INPUT,  GPIO_PULL_UP,   GPIO_2MA), "shtps_irq" },
	{ GPIO_CFG(130, 0, GPIO_OUTPUT, GPIO_NO_PULL,   GPIO_2MA), "shtps_rst" },
};
...
static int msm_shtps_gpio_setup(struct device *dev)
{
	...
	rc = msm_gpios_request_enable(shtps_spi_gpio_config_data,
		ARRAY_SIZE(shtps_spi_gpio_config_data));
...
{
	msm_gpios_disable_free(shtps_spi_gpio_config_data,
		ARRAY_SIZE(shtps_spi_gpio_config_data));
}
...
static struct kgsl_platform_data kgsl_pdata = {
	...
	.max_grp3d_freq = 384000000,
	.min_grp3d_freq = 256000000,
	.set_grp3d_async = NULL,	//NULL
...
static struct msm_acpu_clock_platform_data qsd8x50_clock_data = {
	...
	.max_speed_delta_khz = 256000,	//256000
...
static const struct
{
	...
	{
		{	NB30_KBD_IRQ,   GPIO_CFG(NB30_KBD_IRQ,   0, GPIO_INPUT, GPIO_PULL_UP, GPIO_2MA),  "gpio_keybd_irq"	},
	};
#endif
...
static const struct
{
	...
	{
		{	SH_COMPS_IRQ, GPIO_CFG(SH_COMPS_IRQ,   0, GPIO_INPUT, GPIO_NO_PULL, GPIO_2MA),  "gpio_comps_irq"	},
	};
...
static const struct
{
	...
	{
        {	SH_PROXIMITY_ENABLE, GPIO_CFG(SH_PROXIMITY_ENABLE,   0, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_2MA),  "gpio_proximity_enable"	},
		{	SH_PROXIMITY_IRQ, GPIO_CFG(SH_PROXIMITY_IRQ,   0, GPIO_INPUT, GPIO_PULL_UP, GPIO_2MA),  "gpio_proximity_irq"	},
	};

static void proximity_gpio_release(void)
{
	int i;

	for(i = 0; i < ARRAY_SIZE(msm_i2c_proximity); i++)
	{
		gpio_free(msm_i2c_proximity[i].pinno);
	}
}

static int proximity_gpio_setup(void)
{
	int rc;
	int i;
	...
	for(i = 0; i < ARRAY_SIZE(msm_i2c_proximity); i++)
	{
		rc = gpio_request(msm_i2c_proximity[i].pinno, msm_i2c_proximity[i].msg);
		if (rc) {
			pr_err("[PROXIMITY]gpio_request failed on pin %d (rc=%d)\n", msm_i2c_proximity[i].pinno, rc);
			goto err_gpioconfig;
		}
	}
	for(i = 0; i < ARRAY_SIZE(msm_i2c_proximity); i++)
	{
		rc = gpio_tlmm_config(msm_i2c_proximity[i].cfg, GPIO_ENABLE);
		if (rc) {
			pr_err("[PROXIMITY]gpio_tlmm_config failed on pin %d (rc=%d)\n", msm_i2c_proximity[i].pinno, rc);
			goto err_gpioconfig;
		}
	}
	return rc;
...
err_gpioconfig:
	proximity_gpio_release();
	return rc;
}

static struct sh_i2c_proximity_platform_data msm_proximity_data = {
	.gpio_irq = SH_PROXIMITY_IRQ,
	.gpio_enable = SH_PROXIMITY_ENABLE,
	.gpio_setup = proximity_gpio_setup,
	.gpio_shutdown = proximity_gpio_release,
};
...
static struct i2c_board_info msm_i2c_board_info[] __initdata = {
...
#ifdef CONFIG_KEYBOARD_I2C_MSM
	{
		I2C_BOARD_INFO(NB30_KBD_I2C_DEVNAME, NB30_KBD_I2C_SLAVE),
		.type           = NB30_KBD_I2C_DEVNAME,
		.irq           =  MSM_GPIO_TO_INT(NB30_KBD_IRQ),
...
#endif /* CONFIG_SHFELICA */
	{
		I2C_BOARD_INFO(SHPEM_I2C_DEVNAME, SHPEM_I2C_SLAVE),
		.type           = SHPEM_I2C_DEVNAME,
	},
	{
	...
	{
		I2C_BOARD_INFO(SH_PROXIMITY_I2C_DEVNAME, SH_PROXIMITY_I2C_SLAVE),
		.type           = SH_PROXIMITY_I2C_DEVNAME,
		.irq           =  MSM_GPIO_TO_INT(SH_PROXIMITY_IRQ),
		.platform_data  = &msm_proximity_data
	},
};
...
{
    return sprintf(buf, __stringify(EV_KEY) ":" __stringify(KEY_MENU)   ":83:1033:98:98"
                    ":" __stringify(EV_KEY) ":" __stringify(KEY_HOME)   ":240:1033:98:98"
                    ":" __stringify(EV_KEY) ":" __stringify(KEY_BACK)   ":397:1033:98:98"
                    ":" __stringify(EV_KEY) ":" __stringify(KEY_SEARCH) ":554:1033:98:98"
                    "\n");
}
...
static struct attribute *board_properties_attrs[] = {
    &virtual_keys_attr.attr,
    NULL
};

static struct attribute_group board_properties_attr_group = {
    .attrs = board_properties_attrs,
};

static int virtual_keys_setup(char *name)
{
    int ret = -1;
    struct kobject *kobj;
    virtual_keys_attr.attr.name = name;

    kobj = kobject_create_and_add("board_properties", NULL);
    if (kobj) {
        ret = sysfs_create_group(kobj, &board_properties_attr_group);
    }
...
static void __init qsd8x50_init(void)
{
	...
	virtual_keys_setup("virtualkeys.SH_touchpanel");
	msm_fb_add_devices();
#ifdef CONFIG_MSM_CAMERA
...
MACHINE_START(LYNCIS, "SHARP LYNCIS")
#ifdef CONFIG_MSM_DEBUG_UART
	.phys_io  = MSM_DEBUG_UART_PHYS,
	.io_pg_offst = ((MSM_DEBUG_UART_BASE) >> 18) & 0xfffc,
#endif
