...
define format_kernel_config_engineering
	awk -F '[ =]' 'function no(a){print a"=y"} function yes(a){print "\# "a" is not set"} {if($$1~/^CONFIG_/)name=$$1;else if($$2~/CONFIG_/)name=$$2;else{print; next};if(name~/^CONFIG_DEVK?MEM$$/)no(name);else if(name=="CONFIG_ANDROID_ENGINEERING")no(name);else if(name=="CONFIG_SECURITY_MIYABI")yes(name);else print}' $(1) > $(KERNEL_OUT)/tmp
	rm $(1)
	cp $(KERNEL_OUT)/tmp $(1)
endef
endif
