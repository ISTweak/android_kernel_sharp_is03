...
#define XFER_RETRY   2
static int msm_i2c_sub_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num, int cnt);
...
static struct wake_lock sh_i2c_wake_lock_idle;
static uint32_t sh_i2c_status;
...
#ifdef FEATURE_SHLOCAL_SHI2C
static void msm_i2c_rspin_lock(struct msm_i2c_dev *dev);
static void msm_i2c_rspin_unlock(struct msm_i2c_dev *dev);
...
void sh_i2c_mutex_lock( struct i2c_client *client )
{
	...
	wake_lock( &sh_i2c_wake_lock_idle );
	msm_i2c_rspin_lock(dev);

}
EXPORT_SYMBOL(sh_i2c_mutex_lock);
...
void sh_i2c_mutex_free( struct i2c_client *client )
{
	...
	wake_unlock( &sh_i2c_wake_lock_idle );
	mutex_unlock(&dev->mlock);

}
EXPORT_SYMBOL(sh_i2c_mutex_free);
...
#ifdef FEATURE_SHLOCAL_SHI2C
		if (*smem_ptr == SH_I2C_IDLE_STATE) {
			*smem_ptr = SH_I2C_AARM_RUNNING_STATE;
			gotlock = 1;
		}
#else  /* FEATURE_SHLOCAL_SHI2C */
		if (*smem_ptr == 0) {
...
wait_for_int:
...
#ifdef FEATURE_SHLOCAL_SHI2C
			uint32_t status = readl(dev->base + I2C_STATUS);
			dev_err(dev->dev, "Transaction timed out, status %x\n",status);
#else  /* FEATURE_SHLOCAL_SHI2C */
...
#ifdef FEATURE_SHLOCAL_SHI2C
			dev_err(dev->dev,
				"Error during data xfer (%d), status %x\n",
				dev->err, sh_i2c_status);
#else  /* FEATURE_SHLOCAL_SHI2C */
			dev_err(dev->dev,
 ...
 out_err:
	...
	if( ret < 0){
		...
		remote_spin_lock_irqsave(&dev->s_lock, flags);
		*smem_ptr = SH_I2C_RECOVERY_REQ_STATE;
		remote_spin_unlock_irqrestore(&dev->s_lock, flags);
...
msm_i2c_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&sh_i2c_wake_lock 
					, WAKE_LOCK_SUSPEND
					, "sh_i2c");
	wake_lock_init(&sh_i2c_wake_lock_idle 
					, WAKE_LOCK_IDLE
					, "sh_i2c");
#else  /* FEATURE_SHLOCAL_SHI2C */
...
msm_i2c_remove(struct platform_device *pdev)
{
	...
	wake_lock_destroy( &sh_i2c_wake_lock_idle );
#else  /* FEATURE_SHLOCAL_SHI2C */
