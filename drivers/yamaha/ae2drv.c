...
#define	AE2_USE_VREG_GP1	/* PM[VREG_GP1]control  or  GPIO-SW */

#ifdef AE2_USE_VREG_GP1
#include <mach/vreg.h>
#define AE2_VREG_GP1_LEVEL	(2900)
static struct vreg *vreg_gp1	= NULL;
#endif
...
/* SHARP_EXTEND Start */
#define MA_MAX_ADDRESS				58
static unsigned int gsOpenDriverflg = 0;
/* SHARP_EXTEND End */
...
SetPower( unsigned long on )
{
	if ( on == 0 ) {
		...
		if ( vreg_gp1 != NULL ) {
			...
			printk("SetPower OFF Failed (Not Initialized)\n");
		}
#else
		gpio_direction_output(27, 0);
#endif /* AE2_USE_VREG_GP1 */

#ifdef YAMAHA_DEBUG_LOG
		printk("SetPower GPIO OFF\n");
#endif
	} else {
		...
		if ( vreg_gp1 != NULL ) {
			...
			printk("SetPower ON Failed (Not Initialized)\n");
		}
#else
...
/* SHARP_EXTEND Start */
	if ( gsOpenDriverflg ) return -EMFILE;
/* SHARP_EXTEND End */
	...
	vreg_set_level(vreg_gp1, AE2_VREG_GP1_LEVEL);
#else
