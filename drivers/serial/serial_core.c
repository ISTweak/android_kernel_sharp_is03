...
#define TIOSPSERIAL_FORCE   (0x5522)
...
uart_write(struct tty_struct *tty, const unsigned char *buf, int count)
{
	...
	if (0 == strncmp(tty->name, "ttyMSM", 6) && 2 == port->line) {
	   ...
	   unsigned long arg)
{
...
#ifdef CONFIG_SERIAL_MSM_HS
		struct uart_state *state = tty->driver_data;
		struct uart_port *port = state->uart_port; ret = 0;
	...
	case TIOSPSERIAL_FORCE: /* Force suspend the HW driver */
    {
#ifdef CONFIG_SERIAL_MSM_HS
		struct uart_state *state = tty->driver_data;
		struct uart_port *uport = state->uart_port;
		msm_hs_request_clock_off_force(uport);
		uart_change_pm(state, 3);
#endif
		printk ("uart_ioctl() >> msm_hs_request_clock_off_force()\n");
		ret = 0;
		break;
    }
#endif /* Customize for Bluetooth [001] */
