...
static int msm_startup(struct uart_port *port)
{
	...
	if (port->line == 2) {
		...
		if (tty->driver->termios[idx]) {
			tty->driver->termios[idx]->c_ispeed = 115200;
			tty->driver->termios[idx]->c_ospeed = 115200;
			tty->driver->termios[idx]->c_cflag |= B115200;
			tty->driver->termios[idx]->c_cflag &= ~0xD;
			tty->driver->termios[idx]->c_cflag |= (CLOCAL | CREAD);
			tty->driver->termios[idx]->c_cflag &= ~PARENB;
			tty->driver->termios[idx]->c_cflag &= ~CSTOPB;
			tty->driver->termios[idx]->c_cflag &= ~CSIZE;
			tty->driver->termios[idx]->c_cflag &= ~PARODD;
			tty->driver->termios[idx]->c_cflag |= CS8;
			tty->driver->termios[idx]->c_cflag |= CRTSCTS;

			tty->driver->termios[idx]->c_lflag &= ~(ICANON | IEXTEN | ISIG | ECHO);

			tty->driver->termios[idx]->c_oflag &= ~OPOST;

			tty->driver->termios[idx]->c_iflag &= ~(ICRNL | INPCK | ISTRIP | IXON | BRKINT );
