...
#if 1 /* Customize for Bluetooth [006] */
#include <linux/gpio.h>

/* GPIO mapping for UART Tx/Rx */
#define UART_PORT_RX            45     /* UART1DM_RX_DATA */
#define UART_PORT_TX            46     /* UART1DM_TX_DATA */

/* Port Configuration Table */
static unsigned uart_gpio_config[] = {
    /*        gpio,         func, dir,         pull,         drvstr */
    GPIO_CFG( UART_PORT_RX, 2,    GPIO_INPUT,  GPIO_PULL_UP, GPIO_2MA),
    GPIO_CFG( UART_PORT_TX, 2,    GPIO_OUTPUT, GPIO_PULL_DOWN, GPIO_2MA),
};
#endif /* Customize for Bluetooth [006] */
...
#if 1 /* Customize for Bluetooth [010] */
enum msm_hs_serial_force_sleep_e {
    MSM_HS_SERIAL_FORCE_SLEEP_NONE,
    MSM_HS_SERIAL_FORCE_SLEEP_WAIT,
    MSM_HS_SERIAL_FORCE_SLEEP_DONE,
};

static int msm_hs_force_sleep = MSM_HS_SERIAL_FORCE_SLEEP_NONE;
#endif /* Customize for Bluetooth [010] */
...
#if 1 /* Customize for Bluetooth [010] */
    if (msm_hs_force_sleep != MSM_HS_SERIAL_FORCE_SLEEP_DONE) {
        wake_lock(&msm_uport->dma_wake_lock);
    }
#else /* Customize for Bluetooth [010] */
	wake_lock(&msm_uport->dma_wake_lock);
#endif /* Customize for Bluetooth [010] */
...
#if 1 /* Customize for Bluetooth [010] */
    if (msm_hs_force_sleep == MSM_HS_SERIAL_FORCE_SLEEP_DONE) {
        return 1;   /* force sleep -> normal end */
    }
#endif /* Customize for Bluetooth [010] */
...
#if 1 /* Customize for Bluetooth [006] */
    /* Port Configuration */
    for (pin = 0; pin < ARRAY_SIZE(uart_gpio_config); pin++) {
        result = gpio_tlmm_config( uart_gpio_config[pin], GPIO_DISABLE );
        if (result) {
            printk(KERN_ERR  "gpio_tlmm_config(disable:%d)<-%#x : %d\n", pin, uart_gpio_config[pin], result);
//            return -EIO;
            /* fall-through */
        }
    }
...
#if 1 /* Customize for Bluetooth [010] */
/* request to turn off uart clock once pending TX is flushed */
void msm_hs_request_clock_off_force(struct uart_port *uport) {
    struct msm_hs_port *msm_uport = UARTDM_TO_MSM(uport);
    int pin;
    int result;

    msm_hs_force_sleep = MSM_HS_SERIAL_FORCE_SLEEP_WAIT;
    ...
    wake_unlock(&msm_uport->rx.wake_lock);
    wake_unlock(&msm_uport->dma_wake_lock);
    msm_hs_force_sleep = MSM_HS_SERIAL_FORCE_SLEEP_DONE;
}
EXPORT_SYMBOL(msm_hs_request_clock_off_force);
#endif /* Customize for Bluetooth [010] */
...
static void msm_hs_request_clock_on_locked(struct uart_port *uport) {
	...
	case MSM_HS_CLK_OFF:
        ...
        /* Port Configuration */
        for (pin = 0; pin < ARRAY_SIZE(uart_gpio_config); pin++) {
            result = gpio_tlmm_config( uart_gpio_config[pin], GPIO_ENABLE );
            if (result) {
                printk(KERN_ERR  "gpio_tlmm_config(enable:%d)<-%#x : %d\n", pin, uart_gpio_config[pin], result);
//                return;
                /* fall-through */
            }
        }
...
#if 0 /* Customize for Bluetooth [001] */
	struct msm_serial_hs_platform_data *pdata = pdev->dev.platform_data;
#endif /* Customize for Bluetooth [001] */
#if 1 /* Customize for Bluetooth [006] */
    int pin;
    int result;
#endif /* Customize for Bluetooth [006] */
...
#if 1 /* Customize for Bluetooth [001] */
...
#else /* Customize for Bluetooth [001] */
	if (pdata == NULL)
