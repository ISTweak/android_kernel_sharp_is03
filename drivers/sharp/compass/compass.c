 ...
 *
 * This code is based on akm8975.c.
...
#define AKM8975_SHTERM_ENABLE

#ifdef AKM8975_SHTERM_ENABLE
#include <sharp/shterm_k.h>
#endif	/* AKM8975_SHTERM_ENABLE */
...
static void AKECS_SetYPR(short *rbuf)
{
	...
	if (atomic_read(&a_flag)) {
		tmpX = (rbuf[6] * CONVERT_A_X);
		tmpY = (rbuf[7] * CONVERT_A_Y);
		tmpZ = (rbuf[8] * CONVERT_A_Z);
	...
	if (atomic_read(&mv_flag)) {
		input_report_abs(data->input_dev, ABS_HAT0X, (rbuf[9]  * CONVERT_M_X) );
		input_report_abs(data->input_dev, ABS_HAT0Y, (rbuf[10] * CONVERT_M_Y) );
		input_report_abs(data->input_dev, ABS_BRAKE, (rbuf[11] * CONVERT_M_Z) );
	}
...
static int AKECS_FactoryShipmentTest(void)
{
    ...
    AKMFUNC("AKECS_FactoryShipmentTest");

	/* Set selftest mode 1 */
	buffer[0] = 0x40;
	buffer[1] = AK8975_REG_ASTC;
	...
	mutex_lock(&sense_data_mutex);
	memcpy(buffer, sense_data, SENSOR_DATA_SIZE);
	atomic_set(&data_ready, 0);
	mutex_unlock(&sense_data_mutex);
    ...
    /* -100 <= X <= 100 */
    if((short)buffer[1] < -100 || (short)buffer[1] > 100)
    {
        AKMDBG("E5");
        ret = -EIO;
        goto err_exit;
    ...
    /* -100 <= Y <= 100 */
    if((short)buffer[3] < -100 || (short)buffer[3] > 100)
    {
        AKMDBG("E6");
        ret = -EIO;
        goto err_exit;
    ...
    /* -1000 <= Z <= -300 */
    if((short)buffer[5] < -1000 || (short)buffer[5] > -300)
    {
        AKMDBG("E7");
        ret = -EIO;
        goto err_exit;
...
static int AKECS_GetSensorAdjustData(char *rbuf)
{
	char buffer[3];
	int ret = -1;
	...
	if(AKECS_SetMode(AK8975_MODE_POWERDOWN) >= 0)
	{
		...
		{
			/* Get Adjust data */
			buffer[0] = AK8975_FUSE_ASAX;
			/* Read data */
			if(AKI2C_RxData(buffer, 3) >= 0)
			{
				rbuf[0] = buffer[0];
				rbuf[1] = buffer[1];
				rbuf[2] = buffer[2];
...
static int AKECS_Init_Accuracy(void)
{
	struct akm8975_data *data = i2c_get_clientdata(this_client);

    AKMFUNC("AKECS_Init_Accuracy");
...
static int AKECS_DeviceInitialize(void)
{
	char buffer[2];
	int ret;
		   ...
		   unsigned long arg)
{
	...
	switch (cmd) {
		...
		case ECS_IOCTL_FACTORY_TEST:
			...
			if (ret < 0)
				return ret;
			break;
		case ECS_IOCTL_GET_ADJ_DATA:
			ret = AKECS_GetSensorAdjustData(AdjData);
			if (ret < 0) {
				return ret;
			}
			break;
		case ECS_IOCTL_INIT_ACCURACY:
			ret = AKECS_Init_Accuracy();
			if (ret < 0)
				return ret;
			break;
	...
	switch (cmd) {
		...
		case ECS_IOCTL_GET_DELAY:
			...
			break;
		case ECS_IOCTL_GET_ADJ_DATA:
			if (copy_to_user(argp, &AdjData, sizeof(AdjData))) {
				AKMDBG("copy_to_user failed.");
				return -EFAULT;
			}
			break;
...
static int akm8975_ConfigGPIO(struct sh_i2c_compass_platform_data *poSetupData)
{
	if(poSetupData == NULL)
		return -EINVAL;
...
static int akm8975_ReleaseGPIO(struct sh_i2c_compass_platform_data *poSetupData)
{
	if(poSetupData == NULL)
		return -EINVAL;
	/* Release GPIO */
...
int akm8975_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(akm8975_ConfigGPIO(pdata))
		goto exit2;

	/* Device Initialize */
	err = AKECS_DeviceInitialize();
	if (err < 0) {
		printk(KERN_ERR "AKM8975 akm8975_probe: Device Initialize error\n");
		goto exit3;
	}
	...
	if (err < 0) {
		printk(KERN_ERR "AKM8975 akm8975_probe: Check Device error\n");
		goto exit3;
	}
...
//	register_early_suspend(&akm->akm_early_suspend);	/* not use.  */
...
//	unregister_early_suspend(&akm->akm_early_suspend);	/* not use.  */
...
{
	AKMFUNC("akm8975_init");
	return i2c_add_driver(&akm8975_driver);
}

static void __exit akm8975_exit(void)
{
	AKMFUNC("akm8975_exit");
