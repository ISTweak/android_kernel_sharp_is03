...
#define SHLCDC_IOCTL_GET_UPPER_UNIT _IOR(SHLCDC_IOC_MAGIC, 0, int)

#define SHLCDC_BASE_ADDR 0x60000000
...
struct shlcdc_event {
    int event_type[NUM_SHLCDC_EVENT_TYPE + 1];
};
...
static int shlcdc_upper_init_is_connected = 0;
                         ...
                         unsigned int cmd, unsigned long arg)
{
    ...
    case SHLCDC_IOCTL_SET_VDLINK_MUTEX:
        ...
        break;
    case SHLCDC_IOCTL_GET_UPPER_UNIT:
        ret = copy_to_user(argp,
                           &shlcdc_upper_init_is_connected, sizeof(int));
        if (ret != 0) {
            return ret;
        }
        break;
...
static void shlcdc_set_int_factor(int *event_type)
{
    ...
    if (event_type[NUM_SHLCDC_EVENT_TYPE] == 1) {
        shlcdc_event_info.event_type[NUM_SHLCDC_EVENT_TYPE] = 1;
    }
...
static void shlcdc_get_int_factor(int *event_type)
{
    ...
    if (shlcdc_event_info.event_type[NUM_SHLCDC_EVENT_TYPE] == 1) {
        event_type[NUM_SHLCDC_EVENT_TYPE] = 1;
        shlcdc_event_info.event_type[NUM_SHLCDC_EVENT_TYPE] = 0;
    }
...
static int __init shlcdc_init(void)
{
    ...
    shlcdc_upper_init_is_connected = shdisp_boot_ctx.upper_unit_is_connected;

    shlcdc_base_addr = (unsigned char*)SHLCDC_BASE_ADDR;
