		       ...
		       size_t len, loff_t *ppos)
{
	...
	if((on == SHMFD_PON_STATUS_HIGH) && (pon_high_sts == D_PON_HIGH_DISABLE)){
		MFC_DRV_ERR_LOG("High Sts on = %d, pon_high_sts = %d", on, pon_high_sts); 
		return -EBUSY;
	}
	
	gpio_set_value(D_PON_GPIO_NO , seton );
...
void rfs_poll_work_func(struct work_struct *work)
{
	...
	for (i = 0; i < D_RFS_POLL_SLEEP_NUM; i++) {
		read_value = gpio_get_value(D_RFS_POLL_GPIO_NO);
		if (read_value < 0 || read_value == old_value)
			break;
		
		msleep(D_RFS_POLL_SLEEP_MSEC);
	}
	...
	/* read changed data */
		...
		/* change irq flag */
		if (rfs_d->device_status == D_RFS_DEV_LOW)
			irqflag = IRQF_TRIGGER_HIGH | IRQF_SHARED;
		else
			irqflag = IRQF_TRIGGER_LOW | IRQF_SHARED;
		
		if (set_irq_type(gpio_to_irq(D_RFS_POLL_GPIO_NO), irqflag))
			MFC_DRV_ERR_LOG("set_irq_type irqflag = %ld", irqflag);
	}
	
	/* enable irq handler */
	enable_irq(gpio_to_irq(D_RFS_POLL_GPIO_NO));
	...
	MFC_DRV_DBG_LOG("END read_value = %d, old_value = %d, rfs_d->read_error = %d"
					, read_value, old_value, rfs_d->read_error);
}
...
static irqreturn_t rfs_poll_irq_handler(int irq, void *dev_id)
{
	...
	disable_irq_nosync(gpio_to_irq(D_RFS_POLL_GPIO_NO));
	/* set workqueue */
	schedule_delayed_work(&rfs_d->work, msecs_to_jiffies(D_RFS_POLL_DELAY_MSEC));
...
static int rfs_poll_open(struct inode *inode, struct file *file)
{
	...
	ret = gpio_get_value(D_RFS_POLL_GPIO_NO);
	if (ret < 0) {
		rfs_d->open_flag = 0;
		MFC_DRV_ERR_LOG("gpio_get_value ret = %d", ret);
		return -EIO;
	...
	if (request_irq(gpio_to_irq(D_RFS_POLL_GPIO_NO),
	                rfs_poll_irq_handler,
	                irqflag,
	                D_RFS_POLL_DEV_NAME,
	                (void*)rfs_d)) {
	...
	if (enable_irq_wake(gpio_to_irq(D_RFS_POLL_GPIO_NO))){
		
		MFC_DRV_ERR_LOG("enable_irq_wake");
		
		free_irq(gpio_to_irq(D_RFS_POLL_GPIO_NO), (void *)rfs_d);
...
static int rfs_poll_release(struct inode *inode, struct file *file)
{
	...
	if (disable_irq_wake(gpio_to_irq(D_RFS_POLL_GPIO_NO)))
				...
				size_t count, loff_t *pos)
{
	...
	/* read status register	*/
	ret = felica_ppc_read_register(FELICA_PPC_READ_STATUS_REGISTER_COMMAND, &ppc_data);
...
static int cen_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
