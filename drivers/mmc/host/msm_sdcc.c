...
static struct semaphore	sdio_interrupt_sem;
...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	if (plat->sdiowakeup_irq) {
		init_MUTEX(&sdio_interrupt_sem);

		/* temporal fix for boot issue */
...
#else
			wake_lock_timeout(&host->sdio_wlock, 20);
#endif
