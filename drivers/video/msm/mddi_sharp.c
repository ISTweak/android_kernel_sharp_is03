 ...
 *
 * This code is based on mddi_sharp.c.
...
static int __init mddi_sharp_init(void)
{
	...
	if (mddi_host_core_version > 8) {
		...
		/* dot_clock = 36.80MHz
		 * horizontal count = 666 (642 + 8 + 8 + 8)
		 * vertical count = 970 (960 + 2 + 2 + 6)
		 * refresh rate = 36,800,000/(666*970) = 56.964Hz
		 */
		mddi_sharp_rows_per_second = 55255;	/* 36,800,000/666 */
		mddi_sharp_rows_per_refresh = 970;
		mddi_sharp_usecs_per_refresh = 17555;	/* (666*970)/36,800,000 */
#else
		/* Timing variables for tracking vsync */
		/* dot_clock = 6.00MHz
	...
	if (!ret) {
		...
		pinfo->bpp = 18;
		pinfo->fb_num = 2;
		pinfo->clk_rate = 191000000;	//122880000
		pinfo->clk_min = 190000000;	//120000000
		pinfo->clk_max = 192000000;	//125000000
