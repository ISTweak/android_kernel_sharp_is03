 ...
 *
 * This code is based on mddihosti.c.
...
static void mddi_host_initialize_registers(mddi_host_type host_idx)
{
	...
	mddi_host_reg_out(TA1_LEN, 0x000A);

	/* Turn Around 2 register (= 0x0C) */
	mddi_host_reg_out(TA2_LEN, 0x0034);
	...
	mddi_host_reg_out(REV_RATE_DIV, 0x0004); /* 0x0008 or 0x0004 */
#else
	/* Turn Around 1 register (= 0x05) */
...
#ifdef CONFIG_SHLCDC_BOARD
    mddi_host_reg_out(PAD_CTL, 0x402A8500);
    mddi_host_reg_out(PAD_IO_CTL, 0x00321009);
	mddi_host_reg_out(PAD_CAL, 0x30664020);
#else
#ifdef T_MSM7200
