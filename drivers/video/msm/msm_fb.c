...
static int msm_fb_suspend_flag_shdisp = FALSE;

void msm_fb_suspend_shdisp(int sw)
{
    if (sw == TRUE) {
        msm_fb_suspend_flag_shdisp = TRUE;
        mdelay(100);
    } else {
        msm_fb_suspend_flag_shdisp = FALSE;
    }
    return;
}
#endif
...
static void msm_fb_shutdown(struct platform_device *pdev)
{
    ...
    if (memcmp(pdev->dev.kobj.name, "msm_fb.0", 8) == 0) {
        ret = down_timeout(&shdisp_vdlink_mutex, 30 * HZ);
        if (ret != 0) {
            msm_tps_shutdown();
        } else {
            msm_tps_shutdown();
            ...
            msm_fb_suspend_shdisp(1);
            mdp_suspend_shdisp(1);
            mddi_suspend_shdisp(1);
            shlcdc_api_set_power_mode(SHLCDC_DEV_TYPE_MDDI, SHLCDC_DEV_PWR_OFF);
            shdisp_api_shutdown();
            shlcdc_api_set_power_mode(SHLCDC_DEV_TYPE_HANDSET, SHLCDC_DEV_PWR_OFF);
            up(&shdisp_vdlink_mutex);
        }
    }
...
#ifdef CONFIG_SHLCDC_BOARD
	var->height = 73,	/* height of picture in mm */
...
#ifdef CONFIG_SHLCDC_BOARD
    if (msm_fb_suspend_flag_shdisp == FALSE) {
#endif
