...
#define MSM_NAND_CFG1_RAW 0xD0441

#define MSM_NAND_CFG0_RAW_ONFI_IDENTIFIER 0x88000800
#define MSM_NAND_CFG0_RAW_ONFI_PARAM_INFO 0x88040000
#define MSM_NAND_CFG1_RAW_ONFI_IDENTIFIER 0x000D0441
#define MSM_NAND_CFG1_RAW_ONFI_PARAM_INFO 0x000D0441
...
#define WRITE_PROTECT_AREA_NUM 3
static const char *protect_area_name[]
		= { "systom", "systom", "systom"};
...
#ifndef CONFIG_ANDROID_ENGINEERING
static void msm_nand_set_protect_part(struct platform_device *pdev, struct mtd_info *p_mtd)
{
	uint32_t i,j;

	struct flash_platform_data *pdata = pdev->dev.platform_data;

	memset(s_protect_info, 0x00, sizeof(s_protect_info));

	for(j = 0; j < WRITE_PROTECT_AREA_NUM; j++)
	{
		for (i = 0; i < pdata->nr_parts; i++)
		{
			if(0 == strcmp(pdata->parts[i].name, protect_area_name[j]))
			{
				s_protect_info[j].start  = pdata->parts[i].offset * p_mtd->erasesize;
				s_protect_info[j].end    = pdata->parts[i].offset * p_mtd->erasesize + 
				                           pdata->parts[i].size * p_mtd->erasesize;
				s_protect_info[j].flg    = 1; /* 0:invalid 1:valid */
				break;
			}
		}
	}
...
static int msm_nand_check_protect_part(uint32_t start_adr, uint32_t end_adr)
{
	uint32_t j;

	for(j = 0; j < WRITE_PROTECT_AREA_NUM; j++)
	{
		if(s_protect_info[j].flg == 1)
		{
			/* start address check */
			if((s_protect_info[j].start <= start_adr) && (start_adr < s_protect_info[j].end))
			{
				return -EROFS;
			}

			/* end address check */
			if((s_protect_info[j].start < end_adr) && (end_adr <= s_protect_info[j].end))
			{
				return -EROFS;
			}
		}
...
static struct flash_identification supported_flash[] =
{
	...
	{0x5500b398, 0xFF00FFFF, (1024<<20), 1, 2048, (2048<<6), 64, }, /*Tosh*/
	      ...
	      size_t *retlen, u_char *buf)
{
	int ret;
	struct mtd_oob_ops ops;
	u_char *work_nand_buf= NULL;
	size_t nand_offset_len, nand_part_len, copy_len, left_len;
	...
	if (!dual_nand_ctlr_present) {
		nand_offset_len = from & (mtd->writesize-1);
		nand_part_len = len % mtd->writesize;
		if (nand_offset_len == 0 && nand_part_len == 0) {
			ret = msm_nand_read_oob(mtd, from, &ops);
			*retlen = ops.retlen;
		} else {
			/* read pages with offset or ... */
			*retlen = 0;
	
			work_nand_buf = kmalloc(mtd->writesize, GFP_KERNEL);
			if (!work_nand_buf) {
				pr_err("msm_nand_read: " "failed to alloc work nand buffer\n");
				ret = -ENOMEM;
				goto end_of_offset_or_partial_read;
			}
	
			left_len = len;
	
			if (nand_offset_len != 0) {
				ops.len = mtd->writesize;
				ops.datbuf = work_nand_buf;
				from -= nand_offset_len;
	
				ret = msm_nand_read_oob(mtd, from, &ops);
				if (ops.retlen < mtd->writesize) {
					pr_err("msm_nand_read: " "failed to read page\n");
					goto end_of_offset_or_partial_read;
				}
				
				copy_len = mtd->writesize - nand_offset_len;
				if (copy_len > left_len)
					copy_len = left_len;
	
				memcpy(buf, work_nand_buf+nand_offset_len, copy_len);
				left_len -= copy_len;
				if (left_len == 0) {
					*retlen = len;
					goto end_of_offset_or_partial_read;
			...
			if (left_len > mtd->writesize) {
				copy_len = (left_len / mtd->writesize) * mtd->writesize;
				ops.len = copy_len;
				ops.datbuf = buf;
	
				ret = msm_nand_read_oob(mtd, from, &ops);
				if (ops.retlen < copy_len) {
					pr_err("msm_nand_read: " "failed to read page\n");
					goto end_of_offset_or_partial_read;
				}
			...
			if (ops.retlen < mtd->writesize) {
				...
				memcpy(buf, work_nand_buf, left_len);
				*retlen = len;
			}
	
end_of_offset_or_partial_read:
				kfree(work_nand_buf);
		}
	} else {
...
msm_nand_write_oob(struct mtd_info *mtd, loff_t to, struct mtd_oob_ops *ops)
{
	...
	/* chk_end = start + page_count * 1pageSize */
	if (ops->mode != MTD_OOB_RAW)
		chk_end = to + mtd->writesize * (ops->len / mtd->writesize);
	else
		chk_end = to + mtd->writesize * (ops->len / (mtd->writesize + mtd->oobsize));

	if(0 != msm_nand_check_protect_part(chk_start, chk_end))
	{
		pr_err("%s: WRITE PROTECT ERR start(0x%x) end(0x%x)\n", __func__, chk_start, chk_end);
		return -EROFS;
	}
#endif
...
#ifndef CONFIG_ANDROID_ENGINEERING
	chk_start = instr->addr;
	chk_end = instr->addr + instr->len;
...
msm_nand_block_markbad(struct mtd_info *mtd, loff_t ofs)
{
	...
	if (mtd->writesize == 2048) {
		...
		memset(buf, 0xFF, mtd->writesize + mtd->oobsize);
		buf[mtd->writesize]     = 0xF0;
		buf[mtd->writesize + 1] = 0xF0;
	} else {
		/*
...
#ifndef CONFIG_ANDROID_ENGINEERING
	msm_nand_set_protect_part(pdev, &info->mtd);
#endif
