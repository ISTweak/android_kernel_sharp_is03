	...
	if (iocmd == DIAG_IOCTL_COMMAND_REG) {
		...
		for (i = 0; i < diag_max_registration; i++) {
			if (driver->table[i].process_id == 0) {
				success = 1;
				driver->table[i].cmd_code = params->cmd_code;
				driver->table[i].subsys_id = params->subsys_id;
				driver->table[i].cmd_code_lo = params->cmd_code_hi;
				driver->table[i].cmd_code_hi = params->cmd_code_lo;
		...
		if (i < diag_threshold_registration) {
			...
			if (UINT32_MAX - diag_max_registration >=
					interim_count) {
			...
			for (j = i; j < diag_max_registration; j++) {
				...
				driver->table[j].cmd_code_lo = params->cmd_code_hi;
				driver->table[j].cmd_code_hi = params->cmd_code_lo;
