/* drivers/usb/function/msc_transport.c  (MSC Driver)
 *
			 ...
			 size_t count, loff_t *pos)
{
	...
	while (count > 0) {
		...
		ret = wait_event_interruptible(ctxt->write_wq,
					       (ctxt->error || (req = req_get(ctxt, &ctxt->tx_idle))));
		      ...
		      unsigned int cmd, unsigned long arg)
{
	...
	case USB_MSC_FUNC_IOC_GET_VENDOR_ID:
		if (!arg) {
			printk(KERN_ERR "%s:USB_MSC_FUNC_IOC_GET_VENDOR_ID arg NULL err\n", __func__);
			ret = -EFAULT;
			break;
		}
		if (copy_to_user((void __user*)arg, USB_MSC_VENDOR_ID, USB_MSC_VENDOR_ID_LEN)) {
			printk(KERN_ERR "%s:USB_MSC_FUNC_IOC_GET_VENDOR_ID copy_to_user err\n", __func__);
			ret = -EFAULT;
			break;
		}
	...
	case USB_MSC_FUNC_IOC_GET_PRODUCT_ID:
		if (!arg) {
			printk(KERN_ERR "%s:USB_MSC_FUNC_IOC_GET_PRODUCT_ID arg NULL err\n", __func__);
			ret = -EFAULT;
			break;
		}
		if (copy_to_user((void __user*)arg, USB_MSC_PRODUCT_ID, USB_MSC_PRODUCT_ID_LEN)) {
			printk(KERN_ERR "%s:USB_MSC_FUNC_IOC_GET_PRODUCT_ID copy_to_user err\n", __func__);
			ret = -EFAULT;
			break;
		}
