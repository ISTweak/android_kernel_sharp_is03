/* drivers/usb/function/obex.c  (OBEX Driver)
 *
...
#define OBEX_SEND_PACKET_BUF_SIZE	4096
...
#ifdef CONFIG_SH_USB_CUST
		len = obex_send_packet(dev, req->buf, OBEX_SEND_PACKET_BUF_SIZE);
...
static void obex_write_complete(struct usb_endpoint *ep, struct usb_request *req)
{
	...
	case 0:
		...
		if ((req->length == 0) &&
			(obex_buf_data_avail(port->port_write_buf) == 0)) {
			break;
		}
		if (dev->dev_config)
			obex_send(dev);
...
static void obex_configure(int config, void *_ctxt)
{
	...
	ep = dev->dev_in_ep;
#ifdef CONFIG_SH_USB_CUST
	MaxPacketSize = OBEX_SEND_PACKET_BUF_SIZE;
#else /* CONFIG_SH_USB_CUST */
	MaxPacketSize = usb_ept_get_max_packet(ep);
#endif /* CONFIG_SH_USB_CUST */
