/* drivers/usb/function/msm_hsusb.c  (usb driver specific to msm)
 *
...
#ifdef CONFIG_SH_USB_CUST
#define USB_CHG_DET_DELAY	msecs_to_jiffies(500)
#define USB_SUSPEND_PHY_TIMEOUT	msecs_to_jiffies(500)
...
#ifdef CONFIG_SH_USB_CUST
#define QXDM_FUNCTION_NAME "diag"
#endif /* CONFIG_SH_USB_CUST */
#ifdef CONFIG_SH_USB_CUST
static int pid = USB_PID_MODE3;
#else /* CONFIG_SH_USB_CUST */
static int pid = 0x9018;
#endif /* CONFIG_SH_USB_CUST */

#ifdef CONFIG_SH_USB_CUST
static unsigned long qxdm_enable = 2;
static unsigned long charger_enable = 2;
static unsigned long force_charge = 0;
static unsigned long force_disconnect = 0;
static unsigned long adb_enable = 0;
static unsigned long fs_connect_enable = 0;
static unsigned long hs_connect_enable = 0;
static unsigned long user_pullup = 0;
...
#define USB_USB_GET_MS_OS_DESCRIPTOR		(0xC0)
#define USB_MS_OS_DESCRIPTOR_VENDER			(0x08)
#define USB_MS_OS_DESC_EXT_COMPAT_ID		(0x04)
...
static unsigned char str_mtp_os_desc[] = {
		0x12,		/* length */
		0x03,		/* type */
...
static unsigned char str_mtp_ext_compat_id[] = {
		/* header size=16 */
...
#ifdef CONFIG_SH_USB_CUST
#define USB_MSC_ONLY_FUNC_MAP	0x08 
#else /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
#define MODE_1	"MODE_CDC"
#define MODE_2	"MODE_MTP"
#define MODE_3	"MODE_MSC"
#define MODE_4	"MODE_CDC_2"
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
	struct delayed_work setup0_work;
#endif /* CONFIG_SH_USB_CUST */
	struct delayed_work chg_legacy_det;
#ifdef CONFIG_SH_USB_CUST
	struct delayed_work delay_disconnect_work;	/* delay disable pullup */
#endif /* CONFIG_SH_USB_CUST */
	unsigned phy_status;
...
#ifdef CONFIG_SH_USB_CUST
	fctl_val = ulpi_read(ui, ULPI_FUNCTION_CONTROL_REG);

	if( (fctl_val & 0x18) != 0x08 ) {
		/* if not non-driving mode,then change non-driving mode */
		ulpi_write(ui, (fctl_val & ~0x18 ) | 0x08 , ULPI_FUNCTION_CONTROL_REG);
	}

	chg_val = ulpi_read(ui, ULPI_CHG_DETECT_REG);
	chg_val &= ~0x0f;
	ulpi_write(ui, chg_val | 0x07 , ULPI_CHG_DETECT_REG);
	ulpi_write(ui, chg_val | 0x05 , ULPI_CHG_DETECT_REG);
	udelay(20);	/* wait */
	ulpi_write(ui, chg_val | 0x04 , ULPI_CHG_DETECT_REG);
	udelay(20);	/* wait */
	chg_det = ulpi_read(ui, ULPI_CHG_DETECT_REG);
	ulpi_write(ui, chg_val | 0x0f , ULPI_CHG_DETECT_REG);

	if( (fctl_val & 0x18) != 0x08 ) {
		ulpi_write(ui, fctl_val , ULPI_FUNCTION_CONTROL_REG);
	}
...
#ifdef CONFIG_SH_USB_CUST
static void delay_disconnect(struct work_struct *w)
{
	struct usb_info *ui = the_usb_info;
	unsigned long flags;
	int i;
...
#ifdef CONFIG_SH_USB_CUST
/* recovery */
static void ulpi_err_recovery(void)
{
	struct usb_info *ui = the_usb_info;

	printk(KERN_ERR "ulpi_err_recovery \n" );
...
#ifdef CONFIG_SH_USB_CUST
static void ep0_setup_receive_complete(
		struct usb_endpoint *ept, struct usb_request *req)
{
	struct usb_info *ui = ept->ui;

	ui->ep0out_req->complete = 0;
	ep0_setup_ack(ui);
}
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
	if(len > 0) {
		ui->ep0out_req->complete = ep0_setup_receive_complete;
	}
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
static struct usb_ctrlrequest *store_ctl;



static void usb_do_setup0_work(struct work_struct *w)
{
	struct usb_info *ui = container_of(w, struct usb_info, setup0_work.work);
...
static ssize_t wait_setup_in_response( struct usb_ctrlrequest *pctl )
{
	struct usb_info *ui = the_usb_info;
	...
	store_ctl =  kmalloc( sizeof(struct usb_ctrlrequest), GFP_KERNEL);

	if( store_ctl == 0 )
		return -1;

	memcpy( store_ctl , pctl , sizeof(struct usb_ctrlrequest) );

	queue_delayed_work(usb_work, &ui->setup0_work, 10*HZ );

	return 0;
}

ssize_t send_setup0_in_response(void *buf, ssize_t len )
{
	struct usb_info *ui = the_usb_info;
	struct usb_request *req = ui->setup_req;
	ssize_t count;
	...
	cancel_delayed_work(&ui->setup0_work);

	count = (SETUP_BUF_SIZE < len) ? SETUP_BUF_SIZE : len ;

	memcpy( req->buf, buf , len );
	req->length = len;

	ep0_setup_send(ui, store_ctl->wLength);
...
EXPORT_SYMBOL(send_setup0_in_response);
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
	if( ( ( ctl.bRequestType & USB_TYPE_MASK) == USB_TYPE_VENDOR ) &&
		( ctl.bRequest == USB_MS_OS_DESCRIPTOR_VENDER ) ) {
		unsigned long functions_mask = 0;
		unsigned short pid;
		int i;
		int cnt_port=0;
		...
		if( pid == USB_PID_MODE2 && cnt_port <= 3 ) {
			switch( ctl.wIndex ){
			case USB_MS_OS_DESC_EXT_COMPAT_ID: {
				struct usb_request *req = ui->setup_req;

				str_mtp_ext_compat_id[USB_EXT_COMPAT_ID_INDEX_LENGTH] = 
						USB_EXT_COMPAT_ID_SIZE_HEADER + 
						cnt_port *USB_EXT_COMPAT_ID_SIZE_FUNCTION ;

				str_mtp_ext_compat_id[USB_EXT_COMPAT_ID_INDEX_IFNUM] = 
						cnt_port;

				memcpy( req->buf , str_mtp_ext_compat_id ,
					 sizeof(str_mtp_ext_compat_id));
		...
		if (func && func->setup) {
			if (ctl.bRequestType & USB_DIR_IN) {
				...
				int ret = func->setup(&ctl,
						...
						func->context);
#ifdef CONFIG_SH_USB_CUST
				if( ret == USB_SETUP0_WAIT_SENDING_DATA_RES ) {
					/* store ctl */
					if( wait_setup_in_response(&ctl) )
						goto stall;
					return;
				}
				if (ret == 0) {
					/* do status_stage with out direction */
					ep0_setup_receive(ui,0);
					return;
				}
#endif /* CONFIG_SH_USB_CUST */
				if (ret >= 0) {
...
static irqreturn_t usb_interrupt(int irq, void *data)
{
	...
	if (n & OTGSC_BSVIS) {
		...
		if (B_SESSION_VALID & n)	{
#ifdef CONFIG_SH_USB_CUST
			cancel_delayed_work_sync(&ui->delay_disconnect_work);
#endif /* CONFIG_SH_USB_CUST */
			...
			if( pid == USB_PID_MODE3 ) {
				/* delay for sonyHDR */
				schedule_work(&ui->chg_stop);
				queue_delayed_work(usb_work, &ui->delay_disconnect_work, msecs_to_jiffies(280) );
			}
			else {
				ui->usb_state = USB_STATE_NOTATTACHED;
				ui->configured = 0;
...
#ifdef CONFIG_SH_USB_CUST
	INIT_DELAYED_WORK(&ui->setup0_work, usb_do_setup0_work);
#endif /* CONFIG_SH_USB_CUST */
	INIT_DELAYED_WORK(&ui->chg_legacy_det, usb_chg_legacy_detect);
#ifdef CONFIG_SH_USB_CUST
	INIT_DELAYED_WORK(&ui->delay_disconnect_work, delay_disconnect);	/* delay disable pullup */
#endif /* CONFIG_SH_USB_CUST */
}
...
#ifdef CONFIG_SH_USB_CUST
	/* 500msec timeout */
	timeout_jiffies = jiffies + USB_SUSPEND_PHY_TIMEOUT;
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
					if((!strcmp("adb",fi->func->name))
						&& ( adb_enable == 0 && qxdm_enable == 0 )  ) {
						fi->enabled = 0;
						continue;
					}
					if((!strcmp(QXDM_FUNCTION_NAME,fi->func->name))
...
static void usb_switch_composition(unsigned short pid)
{
	...
	disable_irq(ui->irq);
#ifdef CONFIG_SH_USB_CUST
	if (cancel_delayed_work_sync(&ui->delay_disconnect_work))
		pr_info("%s: Removed delay_disconnect_work successfully\n", __func__);
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
	if(!strcmp("adb",fi->func->name)){
		if ( (hs_connect_enable == 1) || (fs_connect_enable == 1) ) {
			return;
		} else {
			if(adb_enable != enable){
				adb_enable = enable;
			} else {
				return;
			}
...
{
#ifdef CONFIG_SH_USB_CUST
	if (user_pullup == 0 && qxdm_enable == 0 && 
		fs_connect_enable == 0 && hs_connect_enable == 0 ) {
		return;
	}
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
	if(fs_connect_enable) {
		/* Port force full speed connect */
		writel(readl(USB_PORTSC) | PORTSC_PFSC, USB_PORTSC);
	}
	else {
...
#ifdef CONFIG_SH_USB_CUST
	if( charger_enable == ui->selfpowered )	{
		if( charger_enable ) {
			ui->selfpowered = 0;
			ui->maxpower = 0xFA;
		}
...
static void usb_configure_device_descriptor(struct usb_info *ui)
{
#ifdef CONFIG_SH_USB_CUST
	int i;
	unsigned cnt_enable = 0;
	/* When the number of SerialNumber characters is 18 or more,
	...
	if (ui->pdata->serial_number) {
		...
		if(cnt_enable == 1) {
			snprintf(serial_num, sizeof(serial_num), "%-s0",
					ui->pdata->serial_number);
			desc_device.iSerialNumber =
				usb_msm_get_next_strdesc_id(serial_num);
		} else {
			desc_device.iSerialNumber =
...
#ifdef CONFIG_SH_USB_CUST
		if(cnt_enable == 1)
			msm_hsusb_send_serial_number(serial_num);
		else
...
#ifdef CONFIG_SH_USB_CUST
static ssize_t msm_hsusb_show_modeswitch(struct device *dev,
					 struct device_attribute *attr,
					 char *buf)
{
	struct usb_info *ui = the_usb_info;
	...
	unsigned long mode;
	int i; 
	char *mode_name[] = {MODE_1,MODE_2,MODE_3,MODE_4};
...
static ssize_t msm_hsusb_store_modeswitch(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t size)
{
	unsigned long mode;
	unsigned long s_pid[4] = {USB_PID_MODE1,USB_PID_MODE2,USB_PID_MODE3,USB_PID_MODE4};
	struct usb_info *ui = the_usb_info;
	unsigned long functions_mask;
	unsigned short pid;
	int i;
	unsigned char pullup_flag = 0;

	if(((!strncmp(buf, "4", 1)) &&
		(size == 1 || buf[1] == 0x0a || buf[1] == 0)) ||
	    ((!strncmp(buf, MODE_4, strlen(MODE_4))) &&
		(size == strlen(MODE_4) || buf[strlen(MODE_4)] == 0x0a ||
		    buf[strlen(MODE_4)] == 0))) {
		mode = 4;
	}
	else if(((!strncmp(buf, "2", 1)) &&
		(size == 1 || buf[1] == 0x0a || buf[1] == 0)) ||
	    ((!strncmp(buf, MODE_2, strlen(MODE_2))) &&
		(size == strlen(MODE_2) || buf[strlen(MODE_2)] == 0x0a ||
		    buf[strlen(MODE_2)] == 0))) {
		mode = 2;
	}
	else if(((!strncmp(buf, "3", 1)) &&
		(size == 1 || buf[1] == 0x0a || buf[1] == 0)) ||
	    ((!strncmp(buf, MODE_3, strlen(MODE_3))) &&
		(size == strlen(MODE_3) || buf[strlen(MODE_3)] == 0x0a ||
		    buf[strlen(MODE_3)] == 0))) {
		mode = 3;
	}
	else if(((!strncmp(buf, "1", 1)) &&
		(size == 1 || buf[1] == 0x0a || buf[1] == 0)) ||
	    ((!strncmp(buf, MODE_1, strlen(MODE_1))) &&
		(size == strlen(MODE_1) || buf[strlen(MODE_1)] == 0x0a ||
		    buf[strlen(MODE_1)] == 0))) {
		mode = 1;
	}
	else {
		pr_info("%s: Mode Input Error\n", __func__);
		return size;
	}
	...
	user_pullup = 1;
	if( (pid != (unsigned short)s_pid[mode-1]) || (pullup_flag == 1) ){
		usb_switch_composition((unsigned short)s_pid[mode-1]);
	}

	return size;
}

static ssize_t msm_hsusb_show_qxdm_enable(struct device *dev,
						struct device_attribute *attr,
						char *buf)
{
	int i;

	i = scnprintf(buf, PAGE_SIZE, "%ld\n", qxdm_enable);
	return i;
}

static ssize_t msm_hsusb_store_qxdm_enable(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t size)
{
	...
	if (!strict_strtoul(buf, 16, &enable)) {
		...
		usb_function_enable( QXDM_FUNCTION_NAME , (int)enable );
	}
	else {
		pr_info("%s: store_qxdm_enable err \n", __func__);
	}
...
static ssize_t msm_hsusb_show_charger_enable(struct device *dev,
						struct device_attribute *attr,
						char *buf)
{
	int i;
...
static ssize_t msm_hsusb_store_charger_enable(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t size)
{
	unsigned long enable;
	...
	if (!strict_strtoul(buf, 16, &enable)) {
		...
		if( charger_enable != enable )
		{
			charger_enable = enable;

			msm_hsusb_set_chgen(enable);
		}
	}
	else{
		pr_info("%s: store_charger_enable err \n", __func__);
	}

	return size;
}

static ssize_t msm_hsusb_show_force_charge(struct device *dev,
						struct device_attribute *attr,
						char *buf)
{
	int i;
...
static ssize_t msm_hsusb_store_force_charge(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t size)
{
	unsigned long enable;

	strict_strtoul(buf, 16, &enable);
	force_charge = enable;
	if(force_charge){
	  msm_chg_usb_i_is_available(500);
	} else {
	  msm_chg_usb_i_is_not_available();
	}
	return size;
...
static ssize_t msm_hsusb_show_force_disconnect(struct device *dev,
						struct device_attribute *attr,
						char *buf)
{
	int i;
...
static ssize_t msm_hsusb_store_force_disconnect(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t size)
{
	unsigned long enable;
...
static ssize_t usb_full_speed_connect(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t size)
{
	struct usb_info *ui = the_usb_info;
		...
		usb_enable_pullup(ui);
                ...
                pr_info("%s: usb_full_speed_connect err \n", __func__);
        }
        return size;
}
#endif /* CONFIG_SH_USB_CUST */
...
static DEVICE_ATTR(mode, 0664,
		msm_hsusb_show_modeswitch, msm_hsusb_store_modeswitch);
static DEVICE_ATTR(qxdm_enable, 0664,
		msm_hsusb_show_qxdm_enable, msm_hsusb_store_qxdm_enable);
static DEVICE_ATTR(usb_charger_enable, 0664,
		msm_hsusb_show_charger_enable, msm_hsusb_store_charger_enable);
static DEVICE_ATTR(usb_force_charge, 0664,
		msm_hsusb_show_force_charge, msm_hsusb_store_force_charge);
static DEVICE_ATTR(usb_force_disconnect, 0664,
		msm_hsusb_show_force_disconnect, msm_hsusb_store_force_disconnect);
static DEVICE_ATTR(fs_connect, 0664,
		NULL, usb_full_speed_connect);
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
	&dev_attr_mode.attr,
	&dev_attr_qxdm_enable.attr,
	&dev_attr_usb_charger_enable.attr,
	&dev_attr_usb_force_charge.attr,
	&dev_attr_usb_force_disconnect.attr,
	&dev_attr_fs_connect.attr,
#endif /* CONFIG_SH_USB_CUST */
	NULL,
};
...
#ifdef CONFIG_SH_USB_CUST
msm_hsusb_func_attr(obex, 0);
msm_hsusb_func_attr(mdlm, 1);
msm_hsusb_func_attr(modem, 2);
msm_hsusb_func_attr(mass_storage, 3);
msm_hsusb_func_attr(mtp, 4);
msm_hsusb_func_attr(adb, 5);
msm_hsusb_func_attr(diag, 6);
				...
				struct usb_request *req)
{
	...
	if ((type == USB_DT_DEVICE) && (id == 0)) {
		...
		pid = usb_get_product_id(functions_mask);
		if(pid == USB_PID_MODE1 || pid == USB_PID_MODE4) {
...
#define	USB_REMOTE_WAKEUP_SUPPORT	1
	...
	if (type == USB_DT_STRING) {
		...
		if( id == USB_MS_OS_DESCRIPTOR_ID )
		{
			...
			if( pid == USB_PID_MODE2 )
			{
				memcpy(buffer, str_mtp_os_desc , sizeof(str_mtp_os_desc) );
				req->length = buffer[0];
				return 0;
			}
		}
