/* drivers/usb/function/mdlm.c  (MDLM Driver)
 *
...
#define MDLM_SEND_PACKET_BUF_SIZE	4096
...
static void mdlm_init_mdlm_cdc_mdlm_ifc_desc(struct usb_cdc_mdlm_desc *mdlm_desc)
{
	...
	memcpy(mdlm_desc->bGUID, cdc_guid, sizeof(cdc_guid));
...
#ifdef CONFIG_SH_USB_CUST
		len = mdlm_send_packet(dev, req->buf, MDLM_SEND_PACKET_BUF_SIZE);
...
static void mdlm_write_complete(struct usb_endpoint *ep, struct usb_request *req)
{
	...
	case 0:
		...
		if ((req->length == 0) &&
			(mdlm_buf_data_avail(port->port_write_buf) == 0)) {
			break;
		}
		if (dev->dev_config)
			mdlm_send(dev);
...
static void mdlm_configure(int config, void *_ctxt)
{
	...
	ep = dev->dev_in_ep;
#ifdef CONFIG_SH_USB_CUST
	MaxPacketSize = MDLM_SEND_PACKET_BUF_SIZE;
#else /* CONFIG_SH_USB_CUST */
	MaxPacketSize = usb_ept_get_max_packet(ep);
#endif /* CONFIG_SH_USB_CUST */
