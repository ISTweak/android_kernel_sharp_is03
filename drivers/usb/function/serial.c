/* drivers/usb/function/serial.c  (Serial driver)
 *
...
/* Defines */
#ifdef CONFIG_SH_USB_CUST
#define SWITCH_NAME		"switch_modem"
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
#define GS_SEND_PACKET_BUF_SIZE	4096
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
	struct usb_interface_descriptor gs_data_class_desc;
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
static void gs_configure_switch(struct work_struct *w);
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
	.bmCapabilities = 2,  /* bits should be 00000010 (refer to 5.2.3.3) */
#else /* CONFIG_SH_USB_CUST */
...
static void gs_init_ifc_desc(struct usb_interface_descriptor *ifc_desc)
{
	...
	ifc_desc->bInterfaceSubClass =	USB_CDC_SUBCLASS_ACM; /* 0x02 */
...
static void gs_init_data_class_ifc_desc(struct usb_interface_descriptor *ifc_desc)
{
        ifc_desc->bLength = sizeof(struct usb_interface_descriptor);
        ifc_desc->bDescriptorType = USB_DT_INTERFACE; /* 0x04 Interface */
        ifc_desc->bNumEndpoints = 2;
        ifc_desc->bInterfaceClass = USB_CLASS_CDC_DATA;
                                        /* 0x0A [Class] CDC-Data */
...
#ifdef CONFIG_SH_USB_CUST
		ep_desc->wMaxPacketSize = 16;
		if(speed == HIGHSPEED)
			ep_desc->bInterval = 16;
		else
			ep_desc->bInterval = 32;
#else /* CONFIG_SH_USB_CUST */
		ep_desc->wMaxPacketSize = 64;
		ep_desc->bInterval = 4;
#endif /* CONFIG_SH_USB_CUST */
...
static void gs_init_header_desc(struct gs_dev *dev)
{
	...
	dev->gs_highspeed_header[4] =
		(struct usb_descriptor_header *)&gs_union_desc;
	dev->gs_highspeed_header[5] =
		(struct usb_descriptor_header *)&dev->gs_hs_notifyin_desc;
	dev->gs_highspeed_header[6] =
		(struct usb_descriptor_header *)&dev->gs_data_class_desc;
	dev->gs_highspeed_header[7] =
		(struct usb_descriptor_header *)&dev->gs_hs_bulkin_desc;
	dev->gs_highspeed_header[8] =
		(struct usb_descriptor_header *)&dev->gs_hs_bulkout_desc;
	dev->gs_highspeed_header[9] = NULL;
#else /* CONFIG_SH_USB_CUST */
	dev->gs_highspeed_header[1] =
		(struct usb_descriptor_header *)&dev->gs_hs_bulkin_desc;
	dev->gs_highspeed_header[2] =
	...
	dev->gs_fullspeed_header[3] =
		(struct usb_descriptor_header *)&gs_acm_descriptor;
	dev->gs_fullspeed_header[4] =
		(struct usb_descriptor_header *)&gs_union_desc;
	dev->gs_fullspeed_header[5] =
		(struct usb_descriptor_header *)&dev->gs_fs_notifyin_desc;
	dev->gs_fullspeed_header[6] =
		(struct usb_descriptor_header *)&dev->gs_data_class_desc;
	dev->gs_fullspeed_header[7] =
		(struct usb_descriptor_header *)&dev->gs_fs_bulkin_desc;
	dev->gs_fullspeed_header[8] =
		(struct usb_descriptor_header *)&dev->gs_fs_bulkout_desc;
	dev->gs_fullspeed_header[9] = NULL;
#else /* CONFIG_SH_USB_CUST */
	dev->gs_fullspeed_header[1] =
		(struct usb_descriptor_header *)&dev->gs_fs_bulkin_desc;
	dev->gs_fullspeed_header[2] =
...
{
        struct gs_dev  *dev = container_of(sdev, struct gs_dev, sdev);
...
#ifdef CONFIG_SH_USB_CUST
		INIT_WORK(&gs_devices[i]->sw_work, gs_configure_switch);
#endif /* CONFIG_SH_USB_CUST */
		gs_devices[i]->func = func;
#ifdef CONFIG_SH_USB_CUST
		/*6 - Interface, 3 Endpoints-> Total 9 + 1 for NULL*/
		gs_devices[i]->gs_fullspeed_header =
		kmalloc(sizeof(struct usb_descriptor_header *) * 10, GFP_KERNEL);
...
#ifdef CONFIG_SH_USB_CUST
		gs_init_data_class_ifc_desc(&gs_devices[i]->gs_data_class_desc);
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
		gs_devices[i]->sdev.name = SWITCH_NAME;
		gs_devices[i]->sdev.print_name = print_switch_name;
		gs_devices[i]->sdev.print_state = print_switch_state;
		retval = switch_dev_register(&gs_devices[i]->sdev);
		if (retval < 0){
			return retval;
		}
#endif /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
		if(port->port_dev->configured == SERIAL_UNCONFIGURED){
...
#ifdef CONFIG_SH_USB_CUST
		len = gs_send_packet(dev, req->buf, GS_SEND_PACKET_BUF_SIZE);
#else /* CONFIG_SH_USB_CUST */
...
#ifdef CONFIG_SH_USB_CUST
			if ( (prev_len != 0) &&
				 (prev_len%usb_ept_get_max_packet(ep) == 0) ){
#else /* CONFIG_SH_USB_CUST */
...
static void gs_configure_switch(struct work_struct *w)
{
	struct gs_dev *dev = container_of(w,
					      struct gs_dev,
					      sw_work);
...
#ifdef CONFIG_SH_USB_CUST
	intf = usb_msm_get_next_strdesc_id(USB_GS_STRING_DESC_WORD);
	dev->gs_ifc_desc.iInterface = intf;

	gs_union_desc.bMasterInterface0 = ret;
	gs_union_desc.bSlaveInterface0 = ret+1;

	ret = usb_msm_get_next_ifc_number(func);
	dev->gs_data_class_desc.bInterfaceNumber = ret;

	dev->gs_data_class_desc.iInterface = intf;
#else /* CONFIG_SH_USB_CUST */
...
static void gs_configure(int config, void *_ctxt)
{
	...
	ep = dev->dev_in_ep;
#ifdef CONFIG_SH_USB_CUST
	MaxPacketSize = GS_SEND_PACKET_BUF_SIZE;
#else /* CONFIG_SH_USB_CUST */
	MaxPacketSize = usb_ept_get_max_packet(ep);
#endif /* CONFIG_SH_USB_CUST */
